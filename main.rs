...
fn main() {
    ...
    for i in xs {
        let t1 = thread::spawn(move || map_data(&i));
    ...
    //1
    let new_partitions = partition_data(num_partitions, &v);
    //2
    print_partition_info(&new_partitions);
    //3 & 4
    let mut new_intermediates : Vec<usize> = Vec::new();

    for i in new_partitions {
        let t11 = thread::spawn(move || map_data(&i));
        let r11 = t11.join().unwrap();
        new_intermediates.push(r11);
    }
    //5
    println!("Intermediate sums = {:?}", new_intermediates);
    //6
    let new_sum = reduce_data(&new_intermediates);
    //7
    println!("Sum = {}", new_sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    for i in 0..num_partitions{
        let mut x3: Vec<usize> = Vec::new();
        for j in 0+(i*partition_size)..partition_size*(i+1) {
            x3.push(v[j]);
        }
        xs.push(x3);
    }
